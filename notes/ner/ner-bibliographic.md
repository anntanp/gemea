...
**Scale of the fallback**: analysis of 115K Goethe-Faust DDB items ([isbd-title-analysis.md](../../goethe-faust/notes/isbd-title-analysis.md); SPARQL-confirmed in [goethe-faust/notes/corpus-analysis.md §7](../../goethe-faust/notes/corpus-analysis.md)) shows only **~29% of titles carry any ISBD pattern**, meaning NER applies to ~71% of records — the majority, not a small edge case. The dominant ISBD signal is ` :` (18%); the ` / ` split used by the parser appears in only 2.1% of titles. These figures are from one DDB provider subset and may vary across the full corpus. The corpus-wide SR-01 analysis (4.47M records) confirms **71.6% NER fallback** — see [spiel_ner-fallback-coverage.md](ner/spiel_ner-fallback-coverage.md) for the full derivation.
...
The ~28% of records where ISBD area structure is present (`has_dot_dash`) become auto-labeled training examples. Silver candidate selection is handled by `scripts/rate_isbd_fields.py` → `data/processed/sr01_isbd_field_ratings.csv`:
